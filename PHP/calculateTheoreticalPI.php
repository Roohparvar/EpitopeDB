...
function calculateTheoreticalPI($epitope, $tolerance = 0.0001)
{
    $minPH = 0.0;
    $maxPH = 14.0;
    ...
    while (($maxPH - $minPH) > $tolerance) {
        $midPH  = ($minPH + $maxPH) / 2;
        $charge = calculateNetCharge($epitope, $midPH);

        if ($charge > 0) { $minPH = $midPH; } else { $maxPH = $midPH; }
    }

    return ($minPH + $maxPH) / 2;
}